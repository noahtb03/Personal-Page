  ...
  return (
    <div style={{ position: "relative" }}>
          ...
          <div style={{ flex: "1 1 320px", maxWidth: 600 }}>
            ...
            <FadeIn delay={300}>
              <p style={{ fontSize: "clamp(15px, 2vw, 19px)", lineHeight: 1.65, color: COLORS.textMuted, maxWidth: 520, marginTop: 22, fontFamily: "'DM Sans', sans-serif" }}>
                Recent <span style={{ color: COLORS.dark, fontWeight: 600 }}>Ohio State University</span> CSE
                graduate based in New York. Passionate about building impactful software — from AI/ML
                pipelines and API systems to full-stack web platforms.
              </p>
            </FadeIn>

            <FadeIn delay={450}>
              <div style={{ display: "flex", gap: 12, marginTop: 32, flexWrap: "wrap" }}>
                <button onClick={() => setPage("Projects")} style={{ padding: "13px 28px", borderRadius: 100, background: COLORS.accent, color: "#fff", border: "none", fontSize: 14, fontWeight: 600, cursor: "pointer", fontFamily: "'DM Sans', sans-serif", transition: "all 0.3s ease" }}
                  onMouseEnter={(e) => (e.target.style.background = COLORS.accentLight)} onMouseLeave={(e) => (e.target.style.background = COLORS.accent)}>
                  View Projects →
                </button>
                <button onClick={() => setPage("Contact")} style={{ padding: "13px 28px", borderRadius: 100, background: "transparent", color: COLORS.text, border: `1px solid ${COLORS.border}`, fontSize: 14, fontWeight: 500, cursor: "pointer", fontFamily: "'DM Sans', sans-serif", transition: "all 0.3s ease" }}
          ...
          <FadeIn delay={200}>
            <div>
              <p style={{ fontSize: 15, lineHeight: 1.8, color: COLORS.textMuted, margin: 0, fontFamily: "'DM Sans', sans-serif" }}>
                I recently graduated Summa Cum Laude from The Ohio State University with a B.S. in Computer
                Science and Engineering, specializing in Software Engineering. During my time at Ohio State, I
                built an AI classification pipeline that improved model accuracy to 95% and reduced manual
                processing time by 99%, contributed to peer-reviewed research on audio classification models,
                and developed full-stack web platforms.
              </p>
              <p style={{ fontSize: 15, lineHeight: 1.8, color: COLORS.textMuted, margin: 0, marginTop: 18, fontFamily: "'DM Sans', sans-serif" }}>
                Beyond tech, I co-founded the Hip Hop Club at Ohio State — growing it to 400+ members — and
                raised over $1,500 for Hip-Hop for Change, supporting youth in marginalized communities.
                I'm now based in New York and looking for my next opportunity.
              </p>
            </div>
          </FadeIn>
          <div>
            <FadeIn delay={300}>
              <h3 style={{ margin: "0 0 14px 0", fontSize: 13, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.12em", color: COLORS.accent, fontFamily: "'DM Sans', sans-serif" }}>Languages & Frameworks</h3>
              <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>{SKILLS.map((s) => <SkillBadge key={s} name={s} />)}</div>
            </FadeIn>
            <FadeIn delay={350}>
              <h3 style={{ margin: "24px 0 14px 0", fontSize: 13, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.12em", color: COLORS.accent, fontFamily: "'DM Sans', sans-serif" }}>Tools & Environments</h3>
      ...
      <section style={{ padding: "0 clamp(20px, 5vw, 80px) 100px", position: "relative", zIndex: 1 }}>
        <FadeIn delay={100}><SectionTitle label="Experience" title="Where I've Worked" /></FadeIn>
        <div style={{ display: "flex", flexDirection: "column", gap: 14, maxWidth: 800 }}>
          <ExperienceCard role="Innovation Intern" company="American Modern Insurance Group" date="May 2025 – Mar 2026" location="Columbus, OH"
            description="Built and owned an ETL AI classification pipeline using PyTorch, Transformers, Pandas, and NumPy to automate data labeling across ~80,000 conversations, achieving 95% accuracy and reducing manual processing time by 99%. Contributed to enterprise API initiatives with production-ready features. Designed scalable AI architecture that reduced estimated deployment time for future use cases by ~30%."
            delay={200} />
          <ExperienceCard role="AI Research Assistant" company="The Ohio State University" date="Nov 2024 – Jun 2025" location="Columbus, OH"
            description="Collaborated with a research group to implement AI-driven systems supporting post-secondary education. Designed and curated datasets for training and validation workflows to optimize LLM performance. Contributed to a peer-reviewed publication on advanced audio classification models, assisting with experimentation and performance analysis."
            delay={300} />
        </div>
      </section>
        ...
        <FadeIn delay={200}>
          <div style={{ padding: "28px 28px", borderRadius: 14, background: COLORS.bgCard, border: `1px solid ${COLORS.border}`, maxWidth: 800, boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
            ...
            <p style={{ margin: "14px 0 0 0", fontSize: 14, lineHeight: 1.7, color: COLORS.textMuted, fontFamily: "'DM Sans', sans-serif" }}>
              Summa Cum Laude. 7x Dean's List recipient. Co-Founder of Hip Hop Club (400+ members). Coursework in Data Structures & Algorithms, Operating Systems, Software Development & Design, and Language Models.
            </p>
          </div>
        </FadeIn>
      </section>
      ...
      {/* Organizations */}
      <section style={{ padding: "0 clamp(20px, 5vw, 80px) 100px", position: "relative", zIndex: 1 }}>
        <FadeIn delay={100}><SectionTitle label="Organizations" title="Community & Leadership" /></FadeIn>
        <div style={{ display: "flex", flexDirection: "column", gap: 14, maxWidth: 800 }}>
          <OrgCard role="Technical Lead" org="Rise Health Initiative" date="Feb 2026 – Present"
            description="Developed and deployed a full-stack website using React, JSX, HTML, and CSS to improve accessibility to health resources. Designed responsive user interfaces and structured content architecture, leveraging front-end frameworks and accessibility standards for scalable interface design."
            delay={200} />
          <OrgCard role="Co-Founder & Co-Leader" org="Hip Hop Club at The Ohio State University" date="Nov 2022 – Apr 2026"
